...
static const double ROMAX = 58.0;
				...
				if( p_obtenida < grid[cellActual].P_repro ){ //Si la célula actual se reproduce
					...
					for(int indice_vecinos = 0; indice_vecinos < 8; indice_vecinos ++){
						if( grid[cellActual].vecinos[indice_vecinos] != NULL && vecino == 0){
							...
							if( grid[cellActual].vecinos[indice_vecinos]->ro > ROMAX && curand_uniform(&state[rand]) > PS){
...
__global__ void comprobarMadre( Celula *grid, int estado ){
	int x = Madre.x;
	int y = Madre.y;
	if( x*y >= 0 && x < LONG && y < LONG && grid[estado*LONG*LONG + y*LONG + x].ro < ROMAX+1 ){
		if( y-1 >= 0 && x-1 >= 0 && grid[estado*LONG*LONG + (y-1)*LONG + x-1].ro > ROMAX){
			Madre.x = x-1;
			Madre.y = y-1;
		}else if( y-1 >= 0 && grid[estado*LONG*LONG + (y-1)*LONG + x].ro > ROMAX){
			
			Madre.y = y-1;
		}else if( y-1 >= 0 && x+1 < LONG && grid[estado*LONG*LONG + (y-1)*LONG + x+1].ro > ROMAX){
			Madre.x = x+1;
			Madre.y = y-1;
		}else if( x+1 < LONG && grid[estado*LONG*LONG + y*LONG + x+1].ro > ROMAX){
			Madre.x = x+1;
			
		}else if( y+1 < LONG && x+1 < LONG && grid[estado*LONG*LONG + (y+1)*LONG + x+1].ro > ROMAX){
			Madre.x = x+1;
			Madre.y = y+1;
		}else if( y+1 < LONG && grid[estado*LONG*LONG + (y+1)*LONG + x].ro > ROMAX){
			
			Madre.y = y+1;
		}else if( y+1 < LONG && x-1 >= 0 && grid[estado*LONG*LONG + (y+1)*LONG + x-1].ro > ROMAX){
			Madre.x = x-1;
			Madre.y = y+1;
		}else if( x-1 >= 0 && grid[estado*LONG*LONG + y*LONG + x-1].ro > ROMAX){
			Madre.x = x-1;
